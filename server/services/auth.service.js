...
export const setTokenCookies = (res, accessToken, refreshToken) => {
    ...
    res.cookie("refreshToken", refreshToken, { ...cookieOptions, maxAge: 8 * 60 * 60 * 1000 }); // 8 hours
  };
...
export const generateTokens = async (user) => {
    ...
    const expiresIn = new Date(Date.now() + 8 * 60 * 60 * 1000); // Refresh token expiry 8 hours from now
